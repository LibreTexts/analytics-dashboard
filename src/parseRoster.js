...
export default function ParseRoster({ state, setState }) {
  ...
  return (
    <Box direction="column">
      ...
      {state.rosterFile && state.rosterFilterApplied && (
        <Box direction="column" margin={{ top: "small" }}>
          <Box direction="row" align="center" justify="center">
            <Button icon={<FormClose />} onClick={() => removeFile(state, setState)}/>
      ...
      {removeFileLabel &&
        <Text textAlign="center" margin={{top: "small"}}>Hit apply to remove the roster file from the student data.</Text>
      }
    </Box>
  );
}
